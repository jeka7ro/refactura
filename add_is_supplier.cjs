...
async function run() {
  ...
  try {
    await connection.query('ALTER TABLE clients ADD COLUMN isSupplier INT DEFAULT 0');
  ...
  await connection.query('UPDATE clients SET isSupplier = 1 WHERE name LIKE "%JYSK%"');
