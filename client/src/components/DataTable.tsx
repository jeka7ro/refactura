...
export function DataTable<T extends Record<string, any>>({
  ...
  const { sortedData, handleSort, sortColumn, sortDirection, getSortIcon } =
    useTableSort(filteredData, tableId);
  ...
  const handleSelectRow = (row: T) => {
    ...
    setSelectedRows(newSelected);
    const selectedRowsData = paginatedData.filter(r =>
          ...
          <table className="w-full text-xs text-left">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800">
                ...
                <th className="px-3 py-2 w-10 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                  Nr.
                </th>
                {columns.map(col => {
                ...
                {actions && (
                  <th className="px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-slate-400 text-right">
                    ACȚIUNI
                  </th>
                )}
              </tr>
