...
export default function CartScreen() {
  ...
  const suggestions = useMemo(() => {
    ...
    const ADDONS_REGEX =
      /sos|sauce|bautur|drink|desert|dessert|cartof|fries|potato|wedges|soup|supă|salat|miso|ceai|tea|mochi|ketchup|mayo|maionez/i;
    ...
    const candidates = menuProducts.filter(
      p => !cartProductIds.has(p.id) && p.price > 0
    );
    ...
    <div className="cart-screen screen">
      <header className="cart-header">
        ...
        <span className="cart-count-badge">
          {cartItems.length}{" "}
          {cartItems.length > 1 ? t("items_many", lang) : t("item_one", lang)}
        </span>
      </header>
      ...
      <div className="cart-body">
        ...
        <div className="cart-items scroll-y">
                ...
                {items.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="ci-info">
                      ...
                      <span className="ci-unit">
                        {(+item.unitPrice).toFixed(2)} {t("lei", lang)}
                      </span>
                    </div>
          ...
          <button className="add-more-btn" onClick={() => goTo("menu")}>
            {t("add_more", lang)}
          </button>
        </div>
        ...
        <div className="cart-summary">
          ...
          {suggestions.length > 0 && (
            <div className="cart-suggestions">
              <div className="cart-sugg-header">
                <span className="cart-sugg-title">
                  🔥 {t("add_also", lang) || "Adaugă și..."}
                </span>
                <span className="cart-sugg-sub">
                  {t("complete_order", lang) || "Completează comanda ta"}
                </span>
              </div>
          ...
          <div className="summary-rows">
            <div className="summary-row">
              ...
              <span>
                {subtotal.toFixed(2)} {t("lei", lang)}
              </span>
            </div>
            <div className="summary-row">
              <span>
                {t("tva", lang)} {t("tva_included", lang)}
              </span>
              <span>
                {vatAmount.toFixed(2)} {t("lei", lang)}
              </span>
            </div>
              ...
              (async () => {
                try {
                  ...
                  const BACKEND =
                    ...
                    "https://smart-kiosk-v7ws.onrender.com";
                  const res = await fetch(
                    `${BACKEND}/api/promotions/kiosk/${locId}`
                  );
                  const pData = await res.json();

                  if (pData && pData.available && pData.rules) {
                    const trigger =
                      pData.rules.triggerMoment || "after_payment";
                    if (trigger === "before_payment") {
                      const minVal = pData.rules.minOrderValue || 0;
                      const freqEnabled =
                        pData.rules.freqEnabled === undefined
                          ? true
                          : pData.rules.freqEnabled;
                      const ordersToAppear = pData.rules.ordersToAppear || 1;
                      const ordersFinished = parseInt(
                        localStorage.getItem("kiosk_orders_count") || "0",
                        10
                      );

                      const isRightFreq = freqEnabled
                        ? ordersFinished % ordersToAppear === 0
                        : true;
                      const hasSpunTooMany =
                        cartItems.filter(i => i.isPromo).length >= 1;
