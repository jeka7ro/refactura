...
export default function PaymentScreen() {
  ...
  const createOrder = trpc.horeca.orders.create.useMutation();
  const addLine = trpc.horeca.orders.addLine.useMutation();
  const updateStatus = trpc.horeca.orders.updateStatus.useMutation();
      ...
      try {
        ...
        for (const item of cartItems) {
          await addLine.mutateAsync({
            orderId: order.id,
            menuItemId: Number(item.productId) || undefined, // Kiosk ids might be string promo
            quantity: item.quantity,
            unitPrice: String(item.unitPrice),
            notes: item.selectedModifiers?.map(m => m.name).join(", ") || "",
          });
        }
        ...
        console.error("[PaymentScreen] sendOrder failed:", err);
      }
    },
    [cartItems, orderType, createOrder, addLine, updateStatus]
  );
  ...
  const handlePay = useCallback(async () => {
    ...
    try {
      ...
      socket.on(`payment_status_${orderId}`, ({ message }) => {
        ...
        if (lower.includes("banc") || lower.includes("auth"))
          setPayState(STATE.AUTHORIZING);
      });
      ...
      const res = await fetch(`${BACKEND}/api/payment/initiate`, {
        ...
        body: JSON.stringify({
          orderId,
          amount: total,
          paymentGateway: "verifone_serial",
          channel: "kiosk",
        }),
      });
  ...
  const canGoBack = [STATE.IDLE, STATE.ERROR, STATE.DECLINED].includes(
    payState
  );
      ...
      <div className="payment-content fade-in">
          ...
          <>
            ...
            <div className="payment-methods-row">
              <span className="pm-label">
                {t("accepted_methods", lang) || "Metode acceptate"}:
              </span>
              <div className="pm-badges">
                <span className="pm-badge">
                  ...
                  >
                    ...
                    <path
                      d="M7 5.5 C7 3.8 8.2 3 9 3 C9.7 2.5 10.3 2 11 2 C11.7 2 12.2 2.5 12.7 2.5 C13.2 2.5 13.7 2 14.4 2 C15.4 2 16.5 3 16.5 4.5 C16.5 6.5 14.8 8.5 13.5 9 C13 9.5 12.3 9 11.8 9 C11.2 9 10.8 9.5 10.2 9.5 C9.6 9.5 9 9 8.3 8.3 C7.4 7.5 7 6.2 7 5.5Z"
                      fill="white"
                    />
                    <text
                      x="18"
                ...
                <span className="pm-badge">
                  <svg
                    ...
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <rect width="62" height="20" rx="3" fill="#0666EB" />
                    <text
                      x="7"
                      y="14"
                      fontFamily="Arial"
                ...
                <span className="pm-badge">
                  <svg
                    ...
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <rect width="50" height="20" rx="3" fill="#1A1F71" />
                    <text
                      x="6"
                      y="15"
                      fontFamily="Arial"
                ...
                <span className="pm-badge">
                  ...
                  >
                    <rect
                      ...
                      strokeWidth="1"
                    />
                    <circle cx="19" cy="10" r="7.5" fill="#EB001B" />
                    <circle cx="31" cy="10" r="7.5" fill="#F79E1B" />
                    <path
                      d="M25 4.3A7.5 7.5 0 0 1 25 15.7 7.5 7.5 0 0 1 25 4.3Z"
                      fill="#FF5F00"
                    />
                  </svg>
                </span>
            ...
            <button
              className="btn btn-danger btn-lg pay-cancel-btn"
        ...
        {payState === STATE.WAITING_QR && qrUrl && (
          <div className="payment-qr fade-in">
            ...
            <p className="processing-step" style={{ marginBottom: 24 }}>
              Deschide camera telefonului si scaneaza codul de mai jos
            </p>
            <div className="qr-code-wrapper">
              <QRCodeCanvas
                value={qrUrl}
            ...
            <div className="payment-cancel-actions">
              <button className="btn btn-outline btn-lg" onClick={handleCancel}>
                {t("back_to_cart", lang)}
              </button>
              <button
        ...
        {payState === STATE.WAITING_CARD && (
          <div className="payment-processing">
            <div className="pos-nfc-anim">
              <span className="pos-emoji" style={{ fontSize: "4rem" }}>
                📱
              </span>
            ...
            <h2 className="processing-title">
              Apropiați sau introduceți cardul
            </h2>
        ...
        {payState === STATE.PIN_ENTRY && (
          <div className="payment-processing">
            ...
            <p className="processing-step">
              Urmați instrucțiunile de pe terminal
            </p>
            <div className="pin-dots">
              <span className="pin-dot active" />
              <span className="pin-dot active" />
          ...
          <div className="payment-approved fade-in">
            ...
            {txInfo?.authCode && (
              <p className="approved-info">
                Cod: <strong>{txInfo.authCode}</strong>
              </p>
            )}
        ...
        {(payState === STATE.DECLINED || payState === STATE.ERROR) && (
          <div className="payment-declined fade-in">
            <div className="declined-circle">
              {payState === STATE.ERROR ? "!" : "x"}
            </div>
            <h2 className="declined-title">
              {payState === STATE.ERROR ? "Eroare terminal" : "Plata refuzata"}
            </h2>
            <p className="declined-msg">
              Va rugam incercati din nou sau contactati personalul.
            </p>
            <div className="payment-cancel-actions">
              <button className="btn btn-outline btn-lg" onClick={handleRetry}>
                Incearca din nou
              </button>
              <button
