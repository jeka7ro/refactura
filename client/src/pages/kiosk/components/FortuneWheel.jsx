...
export default function FortuneWheel({ config, onClose, onWin }) {
  ...
  const spin = () => {
    ...
    const newRot =
      ...
      (360 - selectedIndex * ANGLE_PER_SLICE) +
      randomOffset;
    ...
    <div className="fortune-wheel-container">
      ...
      <div
        style={{
          ...
          width: 640,
          height: 640,
          marginTop: showWinModal ? -80 : 40,
          transition: "all 0.6s cubic-bezier(0.16, 1, 0.3, 1)",
          transform: showWinModal ? "scale(0.8) translateY(-40px)" : "scale(1)",
          animation: isSpinning ? "none" : "floatWheel 5s infinite ease-in-out",
        }}
      >
        {/* Glow sub roată */}
        <div
          style={{
            ...
            inset: 0,
            borderRadius: "50%",
            background: `radial-gradient(circle, ${winningSlice && showWinModal ? winBg : "#fef08a"}55 0%, transparent 60%)`,
            filter: "blur(40px)",
        ...
        <div
          style={{
            ...
            transition: isSpinning
              ? "transform 5s cubic-bezier(0.25, 1, 0.1, 1)"
              : "none",
          }}
        >
            ...
            {slices.map((slice, i) => {
              ...
              return (
                <g key={`text-${slice.id}`}>
                  {hasImage && (
                    <g
                      transform={`translate(${imgX}, ${imgY}) rotate(${isLeft ? midAngle + 180 : midAngle})`}
                    >
                      <image
                        href={slice.image}
                        x="-6"
        ...
        >
          ...
          <g filter="drop-shadow(-4px 4px 6px rgba(0,0,0,0.6))">
            <path
              d="M 100 44 L 109 46 L 109 54 L 100 56 L 93 50 Z"
              fill="url(#goldRim)"
              stroke="#713f12"
              strokeWidth="0.5"
            />
      ...
      <div
        style={{
          ...
          transition: "all 0.4s",
          opacity: showWinModal ? 0 : 1,
          transform: showWinModal ? "translateY(20px)" : "translateY(0)",
          zIndex: 10,
        }}
      >
        <button
          ...
          style={{
            ...
            boxShadow:
              "0 10px 30px rgba(15,118,110,0.5), inset 0 2px 0 rgba(255,255,255,0.2)",
            transition: "all 0.2s",
            filter: isSpinning ? "grayscale(0.5)" : "none",
          }}
          onMouseDown={e =>
            !isSpinning && (e.currentTarget.style.transform = "scale(0.96)")
          }
          onMouseUp={e =>
            !isSpinning && (e.currentTarget.style.transform = "scale(1)")
          }
          onMouseLeave={e =>
            !isSpinning && (e.currentTarget.style.transform = "scale(1)")
          }
        >
          {isSpinning
            ? t("wheel_spinning", lang)
            : t("wheel_spin_button", lang)}
        </button>
      </div>
      ...
      {showWinModal && winningSlice && (
        <div
          style={{
            ...
            animation: "spinWin 0.6s cubic-bezier(0.16,1,0.3,1)",
            width: "80%",
            maxWidth: 700,
          }}
        >
          {winningSlice.type === "nada" ? (
            <>
              <h2 style={{ fontSize: "2.5rem", margin: 0, fontWeight: 900 }}>
                {t("wheel_no_win_title", lang).replace(
                  "{name}",
                  winningSlice.name
                )}
              </h2>
              <p style={{ fontSize: "1.2rem", opacity: 0.8, marginTop: 10 }}>
                {t("wheel_no_win_sub", lang)}
              </p>
              <button
                onClick={onClose}
                style={{
            ...
            <>
              ...
              <h1
                style={{
                  ...
                  fontWeight: 900,
                  color: "#fff",
                  textShadow: `0 4px 20px ${winBg}88`,
                }}
              >
                {winningSlice.name}
              </h1>
              <p style={{ fontSize: "1.2rem", opacity: 0.9, marginBottom: 30 }}>
                {t("wheel_prize_added", lang)}
              </p>
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    color: "#fff",
                    cursor: "pointer",
                    boxShadow: `0 10px 30px ${winBg}66`,
                  }}
                >
