...
export default function Landing() {
  ...
  const { data: modulesWithPricing = [] } = trpc.public.modulesWithPricing.useQuery();
  ...
  const availableCurrencies = Array.from(
    new Set((modulesWithPricing as any[]).flatMap((m: any) => m.pricing.map((p: any) => p.currency)))
  );
  ...
  const appTabs = [
    { label: "Dashboard", img: APP_SCREENSHOTS.dashboard, desc: "Vizualizare completă: facturi, re-facturi și evoluție." },
    { label: "Facturi Primite", img: APP_SCREENSHOTS.invoices, desc: "Toate facturile furnizorilor centralizate." },
    { label: "Re-Facturi", img: APP_SCREENSHOTS.reinvoices, desc: "Generare re-facturi cu adaos comercial personalizat." },
    { label: "Arhivă", img: APP_SCREENSHOTS.archive, desc: "Arhivă digitală securizată." },
  ];

  const domains = [
    { title: "Construcții & Șantiere", img: DOMAIN_IMAGES.construction, desc: "Gestionează materialele, utilajele și re-facturează subcontractorii direct de pe șantier." },
    { title: "HoReCa & Restaurante", img: DOMAIN_IMAGES.restaurant, desc: "Urmărește facturile de marfă și emite rapoarte financiare instant, de oriunde." },
    { title: "Birouri & Servicii", img: DOMAIN_IMAGES.office, desc: "Automatizează fluxul de facturi pentru clienții tăi corporate și agenții, reducând timpul cu 90%." },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-200">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/80 backdrop-blur-md shadow-sm border-b border-white/20" : "bg-transparent"}`}>
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-10 text-sm font-medium text-slate-600">
            <a href="#features" className="hover:text-blue-600 transition-colors">Funcționalități</a>
            <a href="#domains" className="hover:text-blue-600 transition-colors">Domenii</a>
            <a href="#pricing" className="hover:text-blue-600 transition-colors">Prețuri</a>
          </div>
          <div className="flex items-center gap-4">
            <Button variant="ghost" onClick={() => setLocation("/login")} className="hidden md:flex font-semibold hover:bg-slate-100/50">
              Autentificare
            </Button>
            <Button onClick={() => openTrial()} className="bg-slate-900 hover:bg-slate-800 text-white rounded-full px-6 shadow-xl shadow-slate-900/20 transition-all active:scale-95">
              Încearcă Gratuit
            </Button>
        ...
        {mobileMenu && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="md:hidden bg-white/95 backdrop-blur-xl border-b border-slate-100 px-6 py-6 flex flex-col gap-4 shadow-2xl">
            <a href="#features" onClick={() => setMobileMenu(false)} className="text-lg font-medium">Funcționalități</a>
            <a href="#domains" onClick={() => setMobileMenu(false)} className="text-lg font-medium">Domenii</a>
            <a href="#pricing" onClick={() => setMobileMenu(false)} className="text-lg font-medium">Prețuri</a>
            <hr className="border-slate-100 my-2" />
            <button onClick={() => setLocation("/login")} className="text-lg font-medium text-left">Autentificare</button>
          </motion.div>
        )}
      </nav>
      ...
      <section className="relative pt-20 pb-10 lg:pt-28 lg:pb-16 overflow-hidden flex items-center">
        ...
        <motion.div style={{ y: yHeroBg, opacity: opacityHero }} className="absolute inset-0 -z-10 pointer-events-none">
        ...
        <div className="max-w-5xl mx-auto px-6 flex flex-col items-center text-center w-full relative z-20">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} className="max-w-3xl">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="inline-flex items-center gap-2 px-4 py-2 bg-white/60 backdrop-blur-sm border border-white/50 rounded-full text-sm font-semibold text-blue-700 mb-8 shadow-sm">
            ...
            <h1 className="text-5xl lg:text-7xl font-extrabold text-slate-900 leading-[1.1] mb-8 tracking-tight">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Nu mai greu.</span>
            </h1>
            <p className="text-xl text-slate-600 mb-10 leading-relaxed font-light">
              Importă facturile furnizorilor, adaugă adaosul și generează re-factura PDF în 30 de secunde. Gata cu munca manuală și erorile de calcul.
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <Button onClick={() => openTrial()} size="lg" className="h-14 rounded-full bg-blue-600 hover:bg-blue-700 text-white px-8 text-base font-semibold shadow-xl shadow-blue-600/30 transition-transform hover:scale-105">
            ...
            <div className="mt-8 mb-6 flex items-center justify-center gap-8 text-sm font-medium text-slate-500">
              <div className="flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-green-500" /> Abonament lunar, anulezi oricând</div>
              <div className="flex items-center gap-2"><Shield className="w-5 h-5 text-blue-500" /> Securitate SSL</div>
            </div>
          </motion.div>

          <div className="w-full max-w-7xl mt-4 mb-10 relative z-30 px-4 flex gap-6" style={{height: '520px'}}>
            {/* Mockup - mare pe stanga, aceeasi inaltime */}
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 1, delay: 0.3 }} className="flex-[3] relative z-20 min-w-0 h-full overflow-hidden rounded-2xl">
              <HeroMockup />
            </motion.div>

            {/* 3 carduri stivuite vertical pe dreapta - aceeasi inaltime totala */}
            <div className="flex-[2] flex flex-col gap-3 min-w-0" style={{height: '520px', flexShrink: 0}}>
              {domains.map((domain, i) => (
                <motion.div key={domain.title} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.15 }} className="group relative rounded-2xl overflow-hidden bg-slate-900 shadow-2xl hover:-translate-x-1 transition-transform duration-300 cursor-pointer" style={{flex: 1}}>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    <h3 className="text-xl font-black text-white drop-shadow-lg leading-tight">{domain.title}</h3>
                    <p className="text-white/80 text-sm mt-1 leading-snug line-clamp-2 drop-shadow">{domain.desc}</p>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <section id="features" className="py-32 bg-slate-50 border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-6">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="grid md:grid-cols-3 gap-8">
            {[
              { icon: FileOutput, title: "Modul 1: Re-facturare Automată", desc: "Sincronizare completă cu e-Factura. Transformă automat facturile primite de la furnizori în facturi de vânzare, adăugând adaosul setat." },
              { icon: Building2, title: "Modul 2: Centre de Costuri", desc: "Gestionează bugetele, alocă facturile pe șantiere, proiecte sau departamente și ai claritate totală asupra cheltuielilor fiecărui centru." },
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 justify-center pt-8">
            {(!modulesWithPricing || modulesWithPricing.length === 0) ? (
              [
                { id: 1, name: "Modul 1: Re-facturare", description: "Pentru automatizarea re-facturărilor", icon: FileOutput, pricing: [{ monthlyPrice: "49", currency: "EUR", trialDays: 7 }] },
                { id: 2, name: "Modul 2: Centre de Cost", description: "Gestionare Centre de Cost / Punct de Lucru / Locație", icon: Building2, pricing: [{ monthlyPrice: "79", currency: "EUR", trialDays: 7 }] },
                { id: 3, name: "Combo (Recomandat)", description: "Acces complet la toate funcțiile", icon: Layers, isCombo: 1, pricing: [{ monthlyPrice: "99", currency: "EUR", trialDays: 7 }] }
              ].map((mod, i) => {
                ...
                return (
                  <PricingCard
                    ...
                    price={Number(priceObj.monthlyPrice).toLocaleString("ro-RO")}
                    currency={priceObj.currency}
                    symbol={CURRENCY_SYMBOLS[priceObj.currency] || priceObj.currency}
              ...
              (modulesWithPricing as any[]).map((mod, i) => {
                const priceObj = getPriceForModule(mod, activeCurrency) || mod.pricing[0];
      ...
      <section className="py-32 bg-slate-900 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          <h2 className="text-4xl lg:text-6xl font-bold text-white mb-8 tracking-tight">Pregătit să facturezi corect?</h2>
          <p className="text-xl text-slate-300 mb-12 font-light">Transformă haosul facturilor într-un proces curat, automat și precis. Primele 7 zile sunt din partea noastră.</p>
          <Button onClick={() => openTrial()} size="lg" className="h-16 rounded-full bg-blue-600 hover:bg-blue-500 text-white px-10 text-lg font-bold shadow-2xl shadow-blue-500/50">
      ...
      <footer className="py-12 bg-white border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <img src={LOGO_URL} alt="Refactura" className="w-6 h-6" />
            <span className="font-bold text-slate-900">Refactura.ro</span>
          </div>
          <p className="text-slate-500 text-sm">© {new Date().getFullYear()} Refactura.ro — Construit cu pasiune pentru antreprenori.</p>
          <div className="flex gap-8 text-sm font-medium text-slate-500">
            <a href="#" className="hover:text-blue-600">Termeni & Condiții</a>
      ...
      {/* Trial Modal */}
      {showModal && (
        <TrialModal selectedPlan={selectedPlan} onClose={() => setShowModal(false)} onSuccess={() => { setShowModal(false); setLocation("/register"); }} />
      )}
    </div>
  );
}
...
// ─── Pricing Card ─────────────────────────────────────────────────────────────
function PricingCard({ mod, price, currency, symbol, trialDays, onTrial, highlighted }: any) {
  const IconComp = mod.icon;
  const features = (() => { try { return JSON.parse(mod.features || "[]"); } catch { return []; } })();

  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className={`relative rounded-[2.5rem] p-10 flex flex-col transition-all duration-300 ${highlighted ? "bg-slate-900 text-white shadow-2xl shadow-blue-900/40 -translate-y-4 border border-slate-800" : "bg-white border border-slate-100 shadow-xl shadow-slate-100/60 hover:-translate-y-2"}`}>
      {highlighted && (
        <>
          ...
          <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-10">
            <span className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider shadow-lg shadow-blue-500/30">Alegerea Premium</span>
          </div>
        </>
      )}
      <div className="mb-8 relative z-10">
        ...
        <p className={`text-sm leading-relaxed ${highlighted ? "text-slate-400" : "text-slate-500"}`}>{mod.description}</p>
      </div>
      ...
      <div className="mb-10">
        ...
        <p className={`text-sm mt-3 ${highlighted ? "text-slate-400" : "text-slate-500"}`}>/lună + TVA · {trialDays} zile gratis</p>
      </div>

      <ul className="space-y-4 mb-10 flex-1">
        {features.map((f: string) => (
          <li key={f} className={`flex items-start gap-3 text-sm font-medium ${highlighted ? "text-slate-300" : "text-slate-600"}`}>
...
function TrialModal({ selectedPlan, onClose, onSuccess }: any) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    submitLead.mutate({ ...form, planId: selectedPlan?.id, source: "landing-trial" });
  };

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md" onClick={onClose} />
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-md p-10 z-10">
        ...
        {submitted ? (
          <div className="text-center py-8">
            ...
            <h3 className="text-3xl font-extrabold text-slate-900 mb-4">Felicitări!</h3>
            <p className="text-slate-500 mb-8 leading-relaxed">Cererea ta a fost înregistrată. Un membru al echipei te va contacta în curând pentru activarea contului.</p>
            <Button onClick={onClose} className="w-full h-14 rounded-full bg-slate-900 hover:bg-slate-800 text-white text-base font-bold">Închide</Button>
          </div>
        ) : (
          <>
            <div className="mb-8">
              <h3 className="text-3xl font-extrabold text-slate-900 mb-3">{selectedPlan ? selectedPlan.name : "Trial Gratuit"}</h3>
              <p className="text-slate-500">Introdu datele pentru a începe testarea.</p>
            </div>
  ...
  return (
    <div className="w-full h-full rounded-2xl overflow-hidden shadow-2xl border border-slate-800 bg-slate-50 flex flex-col">
      ...
      <div className="h-9 bg-slate-900 flex items-center px-4 gap-2 flex-shrink-0">
        ...
        <div className="ml-4 text-xs font-medium text-slate-400">app.smartinvoice.ro</div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-44 bg-slate-900 border-r border-slate-800 p-3 flex flex-col gap-1 flex-shrink-0">
          <div className="flex items-center gap-2 mb-4 px-2">
            ...
            <span className="font-bold text-white text-xs">SmartInvoice</span>
          </div>
          <div className={`flex items-center gap-2 px-2 py-2 rounded-lg text-xs font-medium transition-colors ${activeTab === 0 ? "bg-blue-600 text-white" : "text-slate-400 hover:text-white"}`}>
            <BarChart3 className="w-3.5 h-3.5" /> Dashboard
          </div>
          <div className={`flex items-center gap-2 px-2 py-2 rounded-lg text-xs font-medium transition-colors ${activeTab === 1 ? "bg-blue-600 text-white" : "text-slate-400 hover:text-white"}`}>
            <FileText className="w-3.5 h-3.5" /> Re-Facturare
          </div>
          <div className={`flex items-center gap-2 px-2 py-2 rounded-lg text-xs font-medium transition-colors ${activeTab === 2 ? "bg-blue-600 text-white" : "text-slate-400 hover:text-white"}`}>
                ...
                {[40, 60, 45, 80, 65, 90, 100].map((h, i) => (
                  <div key={i} className="flex-1 flex flex-col justify-end items-center gap-1 h-full">
                    <div className="w-full flex gap-0.5 items-end justify-center h-full">
                      <div className="flex-1 bg-blue-500 rounded-t-sm" style={{ height: `${h}%` }} />
                      <div className="flex-1 bg-slate-200 rounded-t-sm" style={{ height: `${h * 0.6}%` }} />
                    </div>
            ...
            <div className="flex gap-4 flex-1">
              ...
              <div className="flex-1 bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl border border-slate-700 shadow-sm p-4 flex flex-col text-white">
                ...
                <div className="grid grid-cols-2 gap-2 border-t border-white/10 pt-3">
                  ...
                  <div>
                    <div className="text-[9px] text-slate-400 uppercase mb-0.5">Costuri</div>
                    <div className="text-xs font-bold text-red-400">375.000 RON</div>
                  </div>
                </div>
              </div>
            </div>
