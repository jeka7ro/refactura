...
function SpvConfigForm({ tenantCui, serverConfigured, hasActiveToken, tokenExpiresAt }: {
  tenantCui?: string;
  ...
  return (
    <div className="pt-3 border-t border-slate-100 dark:border-slate-700 space-y-3">
      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Configurare SPV ANAF</p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        ...
        <div className="relative">
          ...
          />
          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 font-medium">CIF</span>
        </div>
        {/* OAuth Token status */}
        <div className="relative">
          <input
            ...
            placeholder={hasActiveToken ? undefined : "Token lipsă — reconectare necesară"}
          ...
          />
          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 font-medium">Access Token</span>
        </div>
        {/* Client ID server */}
        <div className="relative">
          <input
            ...
            placeholder={serverConfigured ? undefined : "Neconfigurat pe server"}
          ...
          />
          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 font-medium">Client ID/Secret</span>
        </div>
      </div>
      <div className="flex items-center gap-3 flex-wrap">
        <span className={`flex items-center gap-1 text-xs font-medium ${hasActiveToken ? "text-emerald-600" : "text-red-500"}`}>
          {hasActiveToken ? <Check className="w-3.5 h-3.5" /> : <AlertCircle className="w-3.5 h-3.5" />}
          {hasActiveToken
      ...
      <p className="text-[11px] text-slate-400 dark:text-slate-500">
        Conexiunea SPV se face prin OAuth ANAF. Tokenul se reînnoiește automat. CIF-ul este preluat din profilul firmei.
      </p>
    </div>
  );
}
...
function OblioConfigForm({ onSave, saving, initialData }: { onSave: (cfg: Record<string, string>) => void; saving: boolean; initialData?: any }) {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 space-y-3">
      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Configurare Oblio</p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        <input
          type="email"
          value={email}
...
function SmartBillConfigForm({ onSave, saving }: { onSave: (cfg: Record<string, string>) => void; saving: boolean }) {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 space-y-3">
      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Configurare SmartBill</p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        <input
          type="email"
          value={email}
  ...
  return (
    <div className="p-3 sm:p-5 max-w-full space-y-3">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-base font-bold text-slate-900 dark:text-white leading-tight">Integrări</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">Conectează platformele tale de facturare</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "TOTAL INTEGRĂRI", value: PROVIDERS.length, color: "text-slate-700 dark:text-slate-200" },
          { label: "ACTIVE", value: (dbIntegrations as any[]).filter((i: any) => i.status === "active").length, color: "text-emerald-600 dark:text-emerald-400" },
          { label: "NECONECTATE", value: PROVIDERS.length - (dbIntegrations as any[]).filter((i: any) => i.status === "active").length, color: "text-slate-500 dark:text-slate-400" },
          { label: "ULTIMA SYNC", value: (() => { const last = (dbIntegrations as any[]).filter((i: any) => i.lastSyncAt).sort((a: any, b: any) => new Date(b.lastSyncAt).getTime() - new Date(a.lastSyncAt).getTime())[0]; return last ? new Date(last.lastSyncAt).toLocaleDateString("ro-RO") : "—"; })(), color: "text-blue-600 dark:text-blue-400", small: true },
        ].map(s => (
          <div key={s.label} className="bg-white dark:bg-slate-800/60 rounded-lg border border-slate-200 dark:border-slate-700 p-3">
            <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-1">{s.label}</p>
            <p className={`${(s as any).small ? "text-base" : "text-2xl"} font-bold ${s.color}`}>{s.value}</p>
          </div>
        ))}
      </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
              <th className="text-left px-4 py-2.5 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide text-[10px] w-10">NR.</th>
              <th className="text-left px-3 py-2.5 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide text-[10px]">PLATFORMĂ</th>
              <th className="text-left px-3 py-2.5 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide text-[10px]">DESCRIERE</th>
              ...
              <th className="text-left px-3 py-2.5 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide text-[10px]">ULTIMA SYNC</th>
              <th className="text-left px-3 py-2.5 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide text-[10px]">FACTURI</th>
              <th className="text-right px-4 py-2.5 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide text-[10px]">ACȚIUNI</th>
            </tr>
          </thead>
          <tbody>
            {PROVIDERS.map((provider, idx) => {
              ...
              return (
                <>
                  ...
                  >
                    {/* Nr */}
                    <td className="px-4 py-3 text-slate-400 dark:text-slate-500 font-mono text-[11px]">{idx + 1}</td>
                      ...
                      <div className="flex items-center gap-2.5">
                        <div
                          ...
                          style={{
                            ...
                            height: 36,
                            backgroundColor: (provider as any).logoBg || ((provider as any).logoUrl ? "#f8fafc" : provider.logoColor),
                            border: (provider as any).logoUrl ? "1px solid #e2e8f0" : "none",
                          }}
                        >
                          {(provider as any).logoUrl ? (
                            ...
                            <span className="text-white font-bold text-xs">{(provider as any).logoText || provider.name.slice(0, 2)}</span>
                          )}
                        </div>
                        <span className="font-semibold text-slate-900 dark:text-white text-xs">{provider.name}</span>
                      </div>
                    </td>
                        ...
                        <button
                          onClick={() => {
                            if (isActive) {
                              ...
                              // connect
                              if (provider.id === "spv") {
                                if (getSpvOAuthUrl.data?.url) window.location.href = getSpvOAuthUrl.data.url;
                              } else {
                                setConfiguringId(isConfiguring ? null : provider.id);
                              }
                            }
                          }}
                          disabled={disconnectOblioMutation.isPending || disconnectSpvMutation.isPending}
                          ...
                          style={{ backgroundColor: isActive ? "var(--color-primary)" : "#cbd5e1" }}
                        >
                          <span
                    ...
                    <td className="px-3 py-3 text-slate-500 dark:text-slate-400 text-[11px]">
                      {db?.lastSyncAt ? new Date(db.lastSyncAt).toLocaleString("ro-RO", { dateStyle: "short", timeStyle: "short" }) : "—"}
                    </td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex items-center gap-1.5 justify-end flex-nowrap">
                        ...
                        {provider.id === "spv" && !provider.comingSoon && (
                          <>
                            {isActive && (() => {
                              const spvConfigured = !!(getSpvOAuthUrl.data?.serverConfigured && getSpvOAuthUrl.data?.tenantCui);
                              return (
                                <>
                                  <Select
                                    defaultValue="60"
                                    onValueChange={v => (window as any).__spvZile = v}
                                  ...
                                  >
                                    ...
                                    <SelectContent>
                                      ...
                                      <SelectItem value="180">6 luni</SelectItem>
                                  ...
                                  <button
                                    onClick={() => {
                                      if (!spvConfigured) {
                                        toast.error("Configurați mai întâi SPV — CIF, Client ID și Client Secret");
                                        setConfiguringId(provider.id);
                                        return;
                                      }
                                      const zile = parseInt((window as any).__spvZile || "60");
                                    ...
                                    title={!spvConfigured ? "Configurați mai întâi SPV" : "Sincronizează"}
                                  >
                        ...
                        {/* SmartBill */}
                        {provider.id === "smartbill" && !provider.comingSoon && (
                          <>
                            {isActive && (
                              <button
                  ...
                  {/* Config form row */}
                  {isConfiguring && (
                    <tr key={`${provider.id}-cfg`} className="border-b border-slate-100 dark:border-slate-700/60 bg-slate-50/80 dark:bg-slate-800/80">
                      <td />
                      <td colSpan={6} className="px-4 py-3">
                        {provider.id === "spv" && <SpvConfigForm key={db?.id ?? "spv-new"} tenantCui={getSpvOAuthUrl.data?.tenantCui} serverConfigured={getSpvOAuthUrl.data?.serverConfigured} hasActiveToken={!!(db?.apiKey)} tokenExpiresAt={db?.tokenExpiresAt ? String(db.tokenExpiresAt) : null} />}
                        {provider.id === "oblio" && <OblioConfigForm key={db?.id ?? "oblio-new"} initialData={parsedMeta} onSave={saveOblioConfig} saving={upsertMutation.isPending} />}
                        {provider.id === "smartbill" && <SmartBillConfigForm key={db?.id ?? "sb-new"} onSave={saveSmartBillConfig} saving={upsertMutation.isPending} />}
                      </td>
                    </tr>
                  )}
                </>
          ...
          <tfoot>
            <tr className="border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
              <td colSpan={7} className="px-4 py-2 text-[11px] text-slate-400 dark:text-slate-500">
                Total înregistrări: <strong className="text-slate-600 dark:text-slate-300">{PROVIDERS.length}</strong>
                <span className="ml-4">În curând: Saga, WinMentor, Ciel, QuickBooks</span>
              </td>
            </tr>
          </tfoot>
        </table>
      ...
      <AlertDialog open={!!deleteProviderId} onOpenChange={o => !o && setDeleteProviderId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Dezactivează integrarea?</AlertDialogTitle>
            <AlertDialogDescription>
              Această acțiune va deconecta integrarea <strong>{deleteProviderId === "oblio" ? "Oblio" : deleteProviderId === "smartbill" ? "SmartBill" : "SPV ANAF"}</strong> și va șterge token-ul de acces. Facturile importate anterior rămân neafectate.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                if (deleteProviderId === "oblio") disconnectOblioMutation.mutate();
                else if (deleteProviderId === "spv") disconnectSpvMutation.mutate();
              }}
