...
export default function InvoicesEmitted() {
  ...
  const totalVAT = lines.reduce(
    (s, l) => s + lineTotal(l) * (l.vatRate / 100),
    0
  );
  ...
  const selectedClient = (clientsData as any[]).find(
  ...
  const handleSave = async () => {
    ...
    if (!invoiceNumber) {
      toast.error("Completează numărul facturii");
      return;
    }
    if (lines.some(l => !l.description)) {
      toast.error("Completează descrierea la toate liniile");
      return;
    }
    setSaving(true);
    try {
      ...
      toast.success("Factură salvată", {
        description: `Factura ${invoiceNumber} a fost înregistrată.`,
      });
      if (sendToSpv) {
        toast.info("Trimitere SPV", {
          description: "Factura va fi trimisă în SPV ANAF (în lucru).",
        });
      }
      setShowForm(false);
      refetch();
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              Facturi emise de firma ta. Total: <strong>{rows.length}</strong>{" "}
              înregistrări
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
            ...
            <button
              ...
              className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="p-6 space-y-6">
                ...
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 dark:bg-slate-800">
                    <tr>
                      ...
                      <th className="px-3 py-2.5 text-center text-xs font-bold text-slate-500 uppercase w-20">
                        U.M.
                      </th>
                      <th className="px-3 py-2.5 text-right text-xs font-bold text-slate-500 uppercase w-20">
                        Cant.
                      </th>
                      <th className="px-3 py-2.5 text-right text-xs font-bold text-slate-500 uppercase w-28">
                        Preț/U
                      </th>
                      <th className="px-3 py-2.5 text-center text-xs font-bold text-slate-500 uppercase w-20">
                        TVA%
                    ...
                    {lines.map((line, i) => (
                      <tr key={i} className="bg-white dark:bg-slate-900">
                        ...
                        <td className="px-3 py-2 text-right text-sm font-semibold text-slate-900 dark:text-white whitespace-nowrap">
                          {lineTotal(line).toLocaleString("ro-RO", {
                        ...
                        <td className="px-2 py-2">
                          {lines.length > 1 && (
                            <button
                              ...
                              className="w-6 h-6 rounded-lg flex items-center justify-center hover:bg-red-50 dark:hover:bg-red-900/20 text-slate-400 hover:text-red-500 transition-colors"
                            >
            ...
            <div className="flex items-center gap-3 p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
              ...
              <label
                htmlFor="sendSpv"
                className="flex items-center gap-2 text-sm font-medium text-blue-800 dark:text-blue-300 cursor-pointer"
              >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  <th className="px-4 py-3 text-center text-xs font-bold text-slate-500 uppercase w-12">
                    Nr.
                  </th>
                  <th
                    className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase cursor-pointer hover:text-slate-700"
                  ...
                  <th
                    className="px-4 py-3 text-right text-xs font-bold text-slate-500 uppercase cursor-pointer hover:text-slate-700"
                  ...
                  <th
                    className="px-4 py-3 text-center text-xs font-bold text-slate-500 uppercase cursor-pointer hover:text-slate-700"
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {isLoading ? (
                  ...
                  <tr>
                    ...
                    >
                      {search
                        ? "Nicio factură găsită pentru căutarea ta."
                        : "Nicio factură emisă. Apasă «Factură Nouă» pentru a emite."}
                    </td>
                  </tr>
                ) : (
                  paginated.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`px-2.5 py-0.5 rounded-lg text-[11px] font-semibold border ${statusColors[row.status] || statusColors.pending}`}
                        >
