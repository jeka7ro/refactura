...
export default function BonConsumDetail() {
  ...
  const { data: bon, isLoading } = trpc.bonuriConsum.getById.useQuery(
    { id },
    { enabled: !!id }
  );
  ...
  if (!bon)
    return (
      <div className="p-8 text-center text-slate-500">
        Bonul de consum nu a fost găsit.
      </div>
    );
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            Gestiune: {bon.bon.gestiune || "—"}
          </p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <a
            href={`/api/pdf/bon-consum/${bon.bon.id}?download=1`}
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-2 px-4 h-9 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-sm font-bold transition-colors"
          >
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {bon.lines.map((l: any, i: number) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-slate-600 dark:text-slate-300 text-right">
                  {l.quantity}
                </td>
                <td className="px-4 py-3 text-slate-600 dark:text-slate-300 text-right">
                  {l.unitPrice}
