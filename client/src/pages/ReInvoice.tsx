...
import { ArrowLeft, Plus, Trash2, Percent, TrendingUp, Users, Check, Info, Download, Loader2, Search, FileText, ExternalLink } from "lucide-react";
...
export default function ReInvoice() {
  ...
  const parsedIds = idsParam ? idsParam.split(",").map(Number).filter(n => !isNaN(n)) : [];

  const { data: singleData, isLoading: l1 } = trpc.invoiceArchive.getById.useQuery(
    { id: parseInt(id || "0") },
    { enabled: !isMulti && !!id && !isNaN(parseInt(id)) }
  );

  const { data: multiData, isLoading: l2 } = trpc.invoiceArchive.getByIds.useQuery(
    { ids: parsedIds },
    ...
    if (!isMulti) {
      ...
      return {
        ...
        lines: [{
          ...
          quantity: 1,
          unitPrice: parseFloat(String(singleData.totalVAT ? parseFloat(String(singleData.total)) - parseFloat(String(singleData.totalVAT)) : singleData.total || "0")),
          originalUnitPrice: parseFloat(String(singleData.totalVAT ? parseFloat(String(singleData.total)) - parseFloat(String(singleData.totalVAT)) : singleData.total || "0")),
          vatRate: 21,
          unit: "servicii",
          markupPercent: 15,
        }] as any[],
      ...
      multiData.forEach((d, i) => {
        if (d.lines && Array.isArray(d.lines) && d.lines.length > 0) {
          ...
          allLines.push({
            ...
            quantity: 1,
            unitPrice: parseFloat(String(d.totalVAT ? parseFloat(String(d.total)) - parseFloat(String(d.totalVAT)) : d.total || "0")),
            originalUnitPrice: parseFloat(String(d.totalVAT ? parseFloat(String(d.total)) - parseFloat(String(d.totalVAT)) : d.total || "0")),
            vatPercent: 21
          });
        }
      });
      return {
        ...
        supplierName: multiData.map(d => d.supplierName).filter(Boolean).join(", ") || "Furnizori multipli",
        total: multiData.reduce((acc, d) => acc + parseFloat(String(d.total || "0")), 0),
  ...
  interface EditableLine extends Omit<ReInvoiceLine, "quantity" | "unitPrice" | "markupPercent" | "currency"> {
  ...
  const mergeToOneLine = () => {
    ...
    const markupVal = Number(globalMarkup) || 0;
    const totalOriginal = lines.reduce((s, l) => s + (Number(l.quantity) || 0) * (Number(l.originalUnitPrice) || 0), 0);
  ...
  const totalVAT = lines.reduce((s, l) => s + (Number(l.quantity) || 0) * (Number(l.unitPrice) || 0) * ((Number(l.vatRate) || 0) / 100), 0);
  const total = subtotal + totalVAT;
  const originalTotal = invoice?.total || 0;
  const margin = subtotal - (invoice?.lines.reduce((s, l) => s + Number(l.quantity) * Number(l.unitPrice), 0) || 0);
  ...
  const handleSave = async (asDraft: boolean) => {
    ...
    try {
      ...
      const result = await createReInvoice.mutateAsync({
        ...
        sourceInvoiceIds: isMulti && invoice && (invoice as any).sourceInvoiceIds ? (invoice as any).sourceInvoiceIds : undefined,
  ...
  const handleGenerate = async () => {
    if (!clientName.trim()) { toast.error("Numele clientului este obligatoriu!"); return; }
    if (lines.length === 0) { toast.error("Nu există linii de facturat!"); return; }
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      {invoice?.status === 'refactured' && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4 flex gap-3 items-start">
          ...
          <div>
            <h3 className="text-sm font-bold text-amber-800 dark:text-amber-300">Atenție: Factură deja re-facturată</h3>
            <p className="text-xs text-amber-700 dark:text-amber-400/80 mt-1">
              Aceste documente au mai fost re-facturate anterior. Continuarea va crea o nouă re-factură. Poți ignora acest mesaj dacă știi ce faci.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="w-full space-y-4">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
            <div className="p-3 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
              <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">Linii Re-Factură</h3>
            ...
            <div className="grid grid-cols-12 gap-0 bg-[#1e1b4b] dark:bg-slate-800 text-white text-[10px] font-bold uppercase tracking-wider">
              {showCodes ? (
                <>
                  ...
                  <div className="col-span-3 px-3 py-2.5 border-l border-slate-700">Denumire Serviciu</div>
                </>
              ) : (
                <div className="col-span-4 px-3 py-2.5">Denumire Serviciu</div>
              )}
              <div className="col-span-1 px-2 py-2.5 text-center border-l border-slate-700">U.M.</div>
              <div className="col-span-1 px-2 py-2.5 text-center border-l border-slate-700">Cant.</div>
              <div className="col-span-2 px-2 py-2.5 text-right border-l border-slate-700">Preț Orig.</div>
              <div className="col-span-1 px-2 py-2.5 text-center border-l border-slate-700">Adaos %</div>
              <div className="col-span-1 px-2 py-2.5 text-right border-l border-slate-700">Preț Nou</div>
              <div className="col-span-1 px-2 py-2.5 text-right border-l border-slate-700">Total</div>
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {lines.map((line) => {
                ...
                const lineClass = (line as any).isCustom ? "bg-amber-50/50 dark:bg-amber-900/10" : "";

                return (
                  <div key={line.id} className={`grid grid-cols-12 gap-0 items-start hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors ${lineClass}`}>
                    {showCodes && (
                      <div className="col-span-1 px-2 py-2 h-full flex items-center">
                        <input
                          ...
                          onChange={(e) => updateLine(line.id, "devizCode" as any, e.target.value)}
                    ...
                    <div className={`px-2 py-2 relative h-full flex flex-col justify-center ${showCodes ? "col-span-3 border-l border-slate-100 dark:border-slate-800" : "col-span-4"}`}>
                      <input
                        type="text"
                            ...
                            catalogData?.items?.map((item: any, i: number) => (
                              ...
                              >
                                <div className="flex items-center gap-2 mb-0.5">
                                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 uppercase tracking-wider">{item.tip}</span>
                                  <span className="text-xs font-mono text-slate-500">{item.cod}</span>
                                </div>
                    ...
                    <div className="col-span-1 px-1 py-2 border-l border-slate-100 dark:border-slate-800 h-full flex items-center">
                       <input value={line.unit} onChange={(e) => updateLine(line.id, "unit", e.target.value)} className="w-full text-center text-xs bg-transparent focus:outline-none" />
                    </div>
                    <div className="col-span-1 px-1 py-2 border-l border-slate-100 dark:border-slate-800 h-full flex items-center">
                      <input type="number" value={line.quantity} onChange={(e) => updateLine(line.id, "quantity", e.target.value)} className="w-full text-center text-xs bg-transparent focus:outline-none" />
                    </div>
                    ...
                    <div className="col-span-1 px-1 py-2 border-l border-slate-100 dark:border-slate-800 h-full flex items-center">
                      <input type="number" value={line.markupPercent} onChange={(e) => updateLine(line.id, "markupPercent", e.target.value)} className="w-full text-center text-xs bg-transparent focus:outline-none" />
                    </div>
                    <div className="col-span-1 px-1 py-2 border-l border-slate-100 dark:border-slate-800 h-full flex items-center">
                      <input type="number" value={line.unitPrice} onChange={(e) => updateLine(line.id, "unitPrice", e.target.value)} className="w-full text-right text-xs bg-transparent focus:outline-none font-semibold text-blue-600" />
                    </div>
                    ...
                    <div className="col-span-1 flex items-center justify-center border-l border-slate-100 dark:border-slate-800">
                      <button onClick={() => removeLine(line.id)} className="text-slate-400 hover:text-rose-500"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="md:col-span-2 bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:border-slate-800 p-5">
            ...
            <div className="space-y-4">
              <div className="relative">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1.5">Nume sau Cod Fiscal Client *</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1.5">
                    CUI / CIF {cuiLoading && <Loader2 className="w-3 h-3 ml-1 inline animate-spin" />}
                  </label>
                  <input value={clientCUI} onChange={e => setClientCUI(e.target.value)} onBlur={lookupCui} className="w-full h-9 px-3 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1.5">Reg. Com.</label>
                  <input value={clientRegCom} onChange={e => setClientRegCom(e.target.value)} className="w-full h-9 px-3 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1.5">Telefon Client</label>
                  <input value={clientPhone} onChange={e => setClientPhone(e.target.value)} className="w-full h-9 px-3 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1.5">Email</label>
                  <input value={clientEmail} onChange={e => setClientEmail(e.target.value)} className="w-full h-9 px-3 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>

              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-8">
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1.5">Adresă</label>
                  <input value={clientAddress} onChange={e => setClientAddress(e.target.value)} className="w-full h-9 px-3 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div className="col-span-4">
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1.5">Localitate</label>
                  <input value={clientCity} onChange={e => setClientCity(e.target.value)} className="w-full h-9 px-3 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
            </div>
          </div>
