...
export async function createOblioStylePdf(xmlString: string, outputPath: string) {
  ...
  const itemX = 40, umX = 250, qtyX = 300, priceX = 350, valX = 430, tvaX = 500;
  ...
  doc.font('Helvetica');
  doc.text(`Valoare fără TVA:`, 350, totalY + 15); doc.text(subTotal.toString(), 450, totalY + 15, { align: 'right' });
  doc.text(`TVA:`, 350, totalY + 30); doc.text(totalTaxAmt.toString(), 450, totalY + 30, { align: 'right' });
  
  doc.font('Helvetica-Bold').fontSize(12);
  doc.text(`Total de plată:`, 350, totalY + 50); 
  doc.text(`${payable.toString()} ${currency}`, 450, totalY + 50, { align: 'right' });
