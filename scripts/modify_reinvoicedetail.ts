...
content = content.replace(/export default function InvoiceDetail\(\) \{/g, "export default function ReInvoiceDetail() {");

// Change route back link
content = content.replace(/\/facturi-primite/g, "/re-facturi");

// Change tRPC hook
content = content.replace(/trpc\.invoiceArchive\.getById/g, "trpc.reinvoice.getById");

// Change fields
content = content.replace(/invoice\.invoiceNumber/g, "invoice.number");
content = content.replace(/invoice\.supplierName/g, "invoice.sourceSupplierName");
...
content = content.replace(/const isSpv = invoice\.source === "spv_anaf";\n\s*const pdfUrl = isSpv \? `\/api\/pdf\/archive\/\${invoiceId}` : invoice\.fileUrl;/g, 
  'const hasPdf = true;\n        const pdfUrl = `/api/pdf/reinvoice/${invoiceId}`;');

// The fileUrl blocks
content = content.replace(/\{invoice\.fileUrl && invoice\.fileUrl !== "spv_import" && \([\s\S]*?\}\) \(\)\}/g, 
...
content = content.replace(/Vizualizare Factură PDF/g, "Vizualizare Re-Factură PDF");
