...
export function registerPdfRoute(app: any) {
  ...
  router.get("/reinvoice/:id", async (req: Request, res: Response) => {
    try {
      ...
      const [ri] = await db
        .select()
        .from(reInvoices)
        .where(eq(reInvoices.id, id));
      if (!ri) {
        res.status(404).json({ error: "Re-factura nu a fost găsită" });
        return;
      }
      ...
      const [tenant] = await db
        .select()
        .from(tenants)
        .where(eq(tenants.id, ri.tenantId));
      let settings: any = {};
      try {
  ...
  router.get("/archive/:id", async (req: Request, res: Response) => {
    try {
      ...
      const { invoiceArchive, integrations } =
      ...
      if (!xmlContent && inv.source === "spv_anaf" && inv.fileName) {
        console.log(
          `[PDF Archive] rawXml missing for ${inv.invoiceNumber}, attempting re-download...`
        );
          ...
          if (spvIntg?.apiKey) {
            try {
              ...
              const zipRes = await fetch(
                `https://api.anaf.ro/prod/FCTEL/rest/descarcare?id=${downloadId}`,
                {
              ...
              if (zipRes.ok) {
                ...
                if (xmlEntry) {
                  ...
                  await db
                    ...
                    .where(eq(invoiceArchive.id, id));
                  console.log(
                    `[PDF Archive] Re-downloaded and stored XML for ${inv.invoiceNumber}`
                  );
                }
              }
            } catch (dlErr: any) {
              console.error(
                `[PDF Archive] Re-download failed: ${dlErr.message}`
              );
            }
          }
        }
      ...
      if (!xmlContent) {
        res.status(404).json({
          error:
            "XML-ul original nu este disponibil. Mergi la Integrări → SPV → Sincronizează pentru a re-descărca facturile.",
        });
        return;
      }
      ...
      if (!invoiceObj) {
        res.status(500).json({
          error:
            "Invalid SPV XML format: neither Invoice nor CreditNote found.",
        });
        return;
      }
      ...
      const customerParty =
        invoiceObj["cac:AccountingCustomerParty"]?.["cac:Party"];
  ...
  router.get("/emitted/:id", async (req: Request, res: Response) => {
    try {
      ...
      const [inv] = await db
        ...
        .where(eq(emittedInvoices.id, id));
      if (!inv) {
        res.status(404).json({ error: "Factura nu a fost găsită" });
        return;
      }
  ...
  router.get("/nir/:id", async (req: Request, res: Response) => {
    try {
      ...
      if (!nirRow) {
        res.status(404).json({ error: "NIR nu a fost găsit" });
        return;
      }
      ...
      doc
        ...
        .font("Roboto-Bold")
        .fillColor(GRAY)
        .text("FURNIZOR", 50, y + 6);
      doc
        .fontSize(9)
        .font("Roboto-Bold")
        .fillColor("#1e293b")
        .text(nirRow.supplierName || "—", 50, y + 18, { width: halfW - 20 });
      doc
        .fontSize(7)
        .font("Roboto")
        .fillColor(GRAY)
      ...
      doc
        ...
        .font("Roboto-Bold")
        .fillColor(GRAY)
        .text("UNITATEA", x2 + 10, y + 6);
      doc
        .fontSize(9)
        .font("Roboto-Bold")
        .fillColor("#1e293b")
        .text(tenant?.name || "—", x2 + 10, y + 18, { width: halfW - 20 });
      doc
        .fontSize(7)
        .font("Roboto")
        .fillColor(GRAY)
      ...
      doc
        ...
        .font("Roboto-Bold")
        .fillColor(GRAY)
        .text("PRODUSE / SERVICII RECEPȚIONATE", 40, y);
      y += 12;
      ...
      lines.forEach((line, idx) => {
        ...
        const cells = [
          ...
          {
            val: parseFloat(line.cantitateReceptionata || "0").toLocaleString(
              "ro-RO"
            ),
            align: "right" as const,
          },
          {
            val: parseFloat(line.unitPrice || "0").toLocaleString("ro-RO", {
      ...
      if (hasDifferences) {
        ...
        doc
          ...
          .fillColor("#92400e")
          .text(
            "⚠ ATENȚIE: Există diferențe cantitative — NIR se întocmește în 3 exemplare (conf. OMFP 2634/2015)",
            50,
            y + 8
          );
        y += 30;
        if (nirRow.differenceNotes) {
          doc
            ...
            .font("Roboto")
            .fillColor(GRAY)
            .text(`Constatări: ${nirRow.differenceNotes}`, 50, y);
          y += 14;
        }
      }
      ...
      if (members.length > 0) {
        ...
        members.forEach((m, i) => {
          ...
          doc
            ...
            .font("Roboto-Bold")
            .fillColor(GRAY)
            .text(m.func || "Membru", mx + 6, y + 5);
          doc
            .fontSize(8)
            .font("Roboto")
            .fillColor("#1e293b")
            .text(m.name || "", mx + 6, y + 16);
          // Signature line
          doc
            .moveTo(mx + 6, y + 46)
            .lineTo(mx + mW - 20, y + 46)
            .strokeColor("#94a3b8")
            .lineWidth(0.5)
            .stroke();
          doc
      ...
      if (nirRow.notes) {
        ...
        doc
          ...
          .font("Roboto-Bold")
          .fillColor(GRAY)
          .text("OBSERVAȚII:", 40, y);
        doc
          .fontSize(7)
          .font("Roboto")
          .fillColor("#1e293b")
          .text(nirRow.notes, 110, y, { width: W - 70 });
        y += 14;
      }
      ...
      doc
        ...
        .fillColor(GRAY)
        .text(
          `NIR ${nirRow.nirNumber} • Generat: ${new Date().toLocaleDateString("ro-RO")} • ${tenant?.name || ""}`,
          40,
          footerY,
  ...
  router.get("/deviz/:id", async (req: Request, res: Response) => {
    try {
      ...
      if (!deviz) {
        res.status(404).json({ error: "Devizul nu a fost găsit" });
        return;
      }
      ...
      const [tenant] = await db
        .select()
        .from(tenants)
        .where(eq(tenants.id, deviz.tenantId));
      let settings: any = {};
      try {
        settings = JSON.parse(tenant?.settings || "{}");
      } catch {}
      ...
      const robotoReg = pathModule.resolve(
        process.cwd(),
        "server/assets/fonts/Roboto-Regular.ttf"
      );
      const robotoBold = pathModule.resolve(
        process.cwd(),
        "server/assets/fonts/Roboto-Bold.ttf"
      );
      if (fsModule.existsSync(robotoReg)) doc.registerFont("Roboto", robotoReg);
      if (fsModule.existsSync(robotoBold))
        doc.registerFont("Roboto-Bold", robotoBold);
      ...
      doc
        ...
        .font("Roboto-Bold")
        .fillColor(GRAY)
        .text("DATE DEVIZ", 50, y + 6);
      doc
        .fontSize(7)
        .font("Roboto")
        .fillColor(GRAY)
        .text("Nr. Deviz:", 50, y + 20);
      doc
        .fontSize(8)
        .font("Roboto-Bold")
        .fillColor("#1e293b")
        .text(deviz.number, 50, y + 30);
      ...
      doc
        ...
        .font("Roboto-Bold")
        .fillColor("#1e293b")
        .text(new Date(deviz.date).toLocaleDateString("ro-RO"), 150, y + 30);

      doc
        .fontSize(7)
        .font("Roboto")
        .fillColor(GRAY)
        .text("Factura:", 250, y + 20);
      doc
        .fontSize(8)
        .font("Roboto-Bold")
        .fillColor("#1e293b")
        .text(deviz.invoiceId ? `#${deviz.invoiceId}` : "—", 250, y + 30);
  ...
  router.get("/bon-consum/:id", async (req: Request, res: Response) => {
    try {
      ...
      const { bonuriConsum, bonuriConsumLines, tenants } =
        await import("../drizzle/schema");

      const [bon] = await db
        .select()
        .from(bonuriConsum)
        .where(eq(bonuriConsum.id, id));
      if (!bon) {
        res.status(404).json({ error: "Bonul nu a fost găsit" });
        return;
      }
      ...
      const [tenant] = await db
        .select()
        .from(tenants)
        .where(eq(tenants.id, bon.tenantId));
      let settings: any = {};
      try {
        settings = JSON.parse(tenant?.settings || "{}");
      } catch {}
      ...
      doc
        ...
        .font("Roboto-Bold")
        .fillColor(GRAY)
        .text("DATE BON DE CONSUM", 50, y + 6);
      doc
        .fontSize(7)
        .font("Roboto")
        .fillColor(GRAY)
      ...
      doc
        ...
        .font("Roboto-Bold")
        .fillColor("#1e293b")
        .text(bon.number, 50, y + 30);

      doc
        .fontSize(7)
        .font("Roboto")
        .fillColor(GRAY)
        .text("Data eliberării:", 150, y + 20);
      doc
        .fontSize(8)
        .font("Roboto-Bold")
        .fillColor("#1e293b")
        .text(new Date(bon.date).toLocaleDateString("ro-RO"), 150, y + 30);

      doc
        .fontSize(7)
        .font("Roboto")
        .fillColor(GRAY)
        .text("Gestiunea:", 250, y + 20);
      doc
        .fontSize(8)
        .font("Roboto-Bold")
        .fillColor("#1e293b")
        .text(bon.gestiune || "—", 250, y + 30);
      ...
      doc
        ...
        .font("Roboto-Bold")
        .fillColor("#1e293b")
        .text(bon.devizId ? `#${bon.devizId}` : "—", 350, y + 30);
