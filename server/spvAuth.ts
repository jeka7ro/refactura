...
export function registerSpvAuth(app: Express) {
  ...
  app.get("/api/spv/auth", (req, res) => {
    ...
    const redirectUri = process.env.SPV_CALLBACK_URL || `${req.protocol}://${req.get("host")}/api/spv/callback`;

    if (!clientId) {
      return res.status(500).send("SPV_CLIENT_ID is not configured on the server.");
    }
  ...
  app.get("/api/spv/callback", async (req, res) => {
    ...
    if (error) {
      return res.status(400).send(`ANAF Auth Error: ${error} - ${error_description}`);
    }
    ...
    if (!clientId || !clientSecret) {
      return res.status(500).send("SPV credentials not configured on the server.");
    }
    ...
    try {
      ...
      if (!tokenRes.ok) {
        ...
        return res.status(502).send(`Failed to fetch token from ANAF: ${tokenRes.status} ${errText}`);
      }
      ...
      const [existing] = await db
        .select()
        .from(integrations)
        .where(and(eq(integrations.tenantId, tenantId), eq(integrations.provider, "spv")));
      ...
      res.status(500).send(`Internal Server Error during SPV auth: ${err.message}`);
    }
  });
}
