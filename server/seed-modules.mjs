...
const modulesData = [
  {
    ...
    name: "Refacturare",
    description:
      "Importă facturi primite și emite automat re-facturi către clienți. Economisești ore de muncă manuală în fiecare lună.",
    icon: "FileOutput",
    color: "#3B82F6",
    isCombo: 0,
    comboModules: null,
  ...
  },
  {
    ...
    description:
      "Urmărește costurile pe centre de cost, generează rapoarte detaliate și ține evidența bugetelor în timp real.",
    icon: "BarChart3",
    color: "#8B5CF6",
    isCombo: 0,
    comboModules: null,
  ...
  },
  {
    ...
    description:
      "Acces complet la Refacturare + Gestiune Costuri. Soluția completă pentru companii care vor să automatizeze tot fluxul financiar.",
    icon: "Layers",
    color: "#10B981",
    isCombo: 1,
...
for (const mod of modulesData) {
  // Check if exists
  const [rows] = await conn.execute("SELECT id FROM modules WHERE slug = ?", [
    mod.slug,
  ]);
  if (rows.length > 0) {
...
for (const p of pricingData) {
  ...
  if (!moduleId) {
    console.log(`Module ${p.moduleSlug} not found`);
    continue;
  }
  ...
  if (rows.length > 0) {
    console.log(
      `Pricing for '${p.moduleSlug}' (${p.currency}) already exists, skipping.`
    );
    continue;
  }
  await conn.execute(
