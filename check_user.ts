...
async function main() {
  ...
  // Check for the user
  const accountList = await db.select().from(accounts).where(eq(accounts.email, "jeka7ro@gmail.com"));
  ...
  await db.update(accounts).set({ tenantId }).where(eq(accounts.id, account.id));
